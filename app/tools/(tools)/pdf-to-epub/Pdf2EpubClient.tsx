...
import { uint8ArrayToBlob } from "@/lib/utlis/blob";
...
export function Pdf2EpubClient() {
  ...
  return (
    <>
        <ToolShell
            ...
            title="PDF to EPUB"
            subtitle="Create a simple EPUB from extracted PDF text."
        />
                    ...
                    onClick={() =>
                        tool.process(async ([file], onProgress) =>     
                        {
                            ...
                            const output = await epub(
                                { title: baseName(file.name), author: "rescude", publisher: "rescude" },
